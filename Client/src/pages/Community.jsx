     ...
     {image.length > 0 ? (
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {image.map((item , index) => (
            <a key={index} herf={item.imageUrl} className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300'>
              <img src={item.imageUrl} className='w-full h-40 md:h-50 2xl:h-62 object-cover group-hover:scale-105 transition-transform duration-300 ease-in' />
